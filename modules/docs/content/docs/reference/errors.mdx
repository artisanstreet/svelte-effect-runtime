...
type RemoteFailure<Error = unknown> =
	...
	| RemoteValidationError // SER form validation rejected the payload
	| RemoteHttpError // SvelteKit HTTP/App.Error response
...
```ts
yield *
	create_post.submit(data).pipe(Effect.catchTag("YourTag", (err) => Effect.succeed(/* ... */)));
```
...
Remote function schema validation follows SvelteKit's native behavior:
the server calls `handleValidationError`, defaults to `{ message: "Bad Request" }`,
and the client sees a `RemoteHttpError` whose `body` is that `App.Error`
object.

## `EmptyStreamYieldError`

Thrown when generated component code evaluates `yield* stream` and the
stream completes without emitting a value. Use `Stream.runForEach` when
the component should stay subscribed to every value, or make sure the
stream emits at least once before it completes.
