...
type EffectForm<Input, Output, Error = never> =
  ...
  & {
      ...
      preflight(
        schema: StandardSchema | EffectSchema
...
`fields`, `result`, `pending`, the `@attach` symbol) keep their
SvelteKit semantics. `preflight` delegates to SvelteKit after converting
Effect schemas to Standard Schema.
