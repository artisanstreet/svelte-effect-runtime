...
- Calling `ServerRuntime.make(...)` again throws
  `RuntimeAlreadyInitializedError`, except during Vite dev SSR HMR reloads
  where the old runtime is disposed and rebuilt from the reloaded
  `hooks.server.ts` module.
