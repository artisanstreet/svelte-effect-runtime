...
`run_inline_effect` helpers are emitted by the Vite transform; they are
...
again throws `RuntimeAlreadyInitializedError`; configure it once during
startup, before Effect-backed components or browser remote adapters run.

## Internal helpers

These are emitted by the Vite transform; they exist on the public surface
