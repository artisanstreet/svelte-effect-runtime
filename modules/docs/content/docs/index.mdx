...
description: Install and configure Effect-native helpers for SvelteKit.
---
...
<div className="rounded-md border bg-fd-muted/40 p-4 text-sm">
...
[Query](/docs/remote-functions/query), [Form](/docs/remote-functions/form),
[Command](/docs/remote-functions/command), and
[Prerender](/docs/remote-functions/prerender) functions.

## Effect version

`svelte-effect-runtime` V2 targets Effect v4 beta. Import runtime APIs from the
package root and import Effect APIs directly from `effect`:
...
<div className="rounded-md border bg-fd-muted/40 p-4 text-sm">
  Registering a runtime in <code>src/hooks.client.ts</code> or{" "}
  <code>src/hooks.server.ts</code> is optional. A default empty-layer runtime is
  created lazily when nothing has been registered yet.
</div>
