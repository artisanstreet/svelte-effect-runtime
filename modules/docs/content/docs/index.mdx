...
Install the package, enable Svelte async rendering, and add the Vite plugin before `sveltekit()`:
...
- Add `effect()` in `vite.config.ts`; that is what lowers component `yield*`.
