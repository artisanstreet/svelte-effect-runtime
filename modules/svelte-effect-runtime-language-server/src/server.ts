...
const invoked_module_path =
	process.argv[1] === undefined ? undefined : realpathSync.native(path.resolve(process.argv[1]));
...
function create_language_server_connection(): Connection {
	if (process.argv.includes("--stdio")) {
		...
		const connection = createConnection(process.stdin, process.stdout) as Connection;
...
function patch_pull_diagnostics_connection(connection: Connection): Connection {
