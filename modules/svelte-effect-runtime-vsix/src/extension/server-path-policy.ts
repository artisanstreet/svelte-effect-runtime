...
export function resolve_configured_server_path(
  ...
  const global_path = normalize_configured_server_path(
    configuration.global_path,
  );
  const ignored_workspace_path = get_workspace_configured_server_path(
    configuration,
  );
  const invalid_global_path = global_path &&
      !is_safe_language_server_path(global_path)
    ? global_path
    : undefined;
 ...
 */
export function normalize_configured_server_path(
  value: unknown,
): string | undefined {
...
export function is_safe_language_server_path(server_path: string): boolean {
  ...
  return normalized_path.length > 0 &&
...
export function assert_safe_language_server_path(server_path: string): void {
  ...
  throw new Error(
    "Language-server path must be an absolute local filesystem path.",
  );
}
